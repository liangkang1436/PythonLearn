...
# 如果被迭代的容器对象的元素本身也可以迭代，则可以使用多个对象来承接，而如果被迭代的容器对象的元素无法迭代，但是你用多个对象承接，则会报错
...
# 会尝试迭代容器对象（列表，元组，集合，字典）的每一个元素，然后顺序赋值给声明的变量，比如char1, char2
# 输出 a-b c-d e-f g-h
...
# 如果容器对象的元素本身（是数字或者Boolean，而不是字符串、列表元组，集合，字典），无法迭代，则会报错
